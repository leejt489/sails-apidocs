...
function _getRoutes() {
	...
	//Starting with the models and then adding controllers that do not have models, build a collection of resources
	var resourceNames = Object.keys(models);
	Object.keys(controllers).forEach(function(controller) {
		...
		if (resourceNames.indexOf(t) > -1)
			resourceNames.push(t);
	});
...
function _mergeDocs() {
	...
	Object.keys(resources).forEach(function(resourceName) {
		...
		if (routes.length > 0) {
			resourceDocs = resourceDocs.concat(_modelDocs[resourceName]);
